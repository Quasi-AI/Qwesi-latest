    ...
    return (
        <footer className="mx-6 bg-white">
            <div className="max-w-7xl mx-auto">
                <div className="flex flex-col md:flex-row items-start justify-between gap-10 py-10 border-b border-gray-200 text-gray-600">
                    <div>
                        ...
                        <div className="flex items-center gap-3 mt-5">
                            {socialIcons.map((item, i) => (
                                <Link href={item.link} key={i} className="flex items-center justify-center w-10 h-10 bg-white border border-[#432DD7] hover:bg-[#432DD7] hover:scale-105 transition rounded-full group">
                                    <div className="[&>svg>path]:group-hover:stroke-white">
                                        <item.icon />
                                    </div>
                                </Link>
                            ))}
                    ...
                    <div className="flex flex-wrap justify-between w-full md:w-[45%] gap-5 text-sm ">
                        {linkSections.map((section, index) => (
                            <div key={index}>
                                <h3 className="font-medium text-[#432DD7] md:mb-5 mb-3">{section.title}</h3>
                                <ul className="space-y-2.5">
                                    {section.links.map((link, i) => (
                                        <li key={i} className="flex items-center gap-2">
                                            {link.icon && <link.icon />}
                                            <Link href={link.path} className="hover:text-[#432DD7] transition">{link.text}</Link>
                                        </li>
                                    ))}
                                </ul>
                            </div>
                ...
                <div className="py-4 flex flex-col sm:flex-row justify-between items-center text-sm text-gray-500">
                    <p>Copyright 2025 © Qwesi AI. All Rights Reserved.</p>
                    <div className="flex gap-4 mt-2 sm:mt-0">
                        <Link href="/privacy" className="hover:text-[#432DD7] transition">Privacy Policy</Link>
                        <Link href="/terms" className="hover:text-[#432DD7] transition">Terms of Service</Link>
                    </div>
                </div>
            </div>
        </footer>
